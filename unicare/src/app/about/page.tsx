import Image from "next/image";

export default function AboutPage() {
    return (
        <div className="bg-background-light min-h-screen pt-28 pb-20">
            {/* Hero Section - Split Layout */}
            <section className="max-w-6xl mx-auto px-4 md:px-8 py-10">
                <div className="flex flex-col gap-10 lg:flex-row lg:items-center">
                    {/* Left: Hero Image */}
                    <div className="w-full lg:w-1/2 relative group overflow-hidden rounded-xl">
                        <div className="absolute inset-0 bg-primary/10 z-10 pointer-events-none mix-blend-multiply" />
                        <div className="relative w-full h-[300px] md:h-[500px] overflow-hidden">
                            <Image
                                src="/about hero.png"
                                alt="Artistic sketch of engineering blueprints and connecting lines representing community"
                                fill
                                className="object-cover transition-transform duration-700 group-hover:scale-105"
                                priority
                            />
                        </div>
                        {/* Overlay Badge */}
                        <div className="absolute bottom-6 left-6 z-20 bg-white/90 backdrop-blur-sm p-4 rounded-lg border border-primary/20">
                            <p className="text-xs font-bold uppercase tracking-widest text-primary mb-1">Est. 2024</p>
                            <p className="text-sm text-neutral-800">Designing the future, together.</p>
                        </div>
                    </div>

                    {/* Right: Mission Text */}
                    <div className="w-full lg:w-1/2 flex flex-col gap-6 lg:pl-12">
                        <div className="flex flex-col gap-2 text-left">
                            <span className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold w-fit mb-2">
                                OUR STORY
                            </span>
                            <h1 className="text-4xl md:text-5xl font-black leading-tight tracking-tight text-neutral-900">
                                Redefining Colleagues through Connection
                            </h1>
                            <p className="text-neutral-500 text-lg font-normal leading-relaxed mt-4">
                                UniCare isn&apos;t just a platform; it&apos;s a movement. We are democratizing engineering resources
                                through a soft, welcoming, and artistic lens. In a world of rigid code, we find the art in
                                collaboration.
                            </p>
                            <p className="text-neutral-500 text-base font-normal leading-relaxed">
                                Our community-driven resource-sharing platform is designed specifically for engineers who value open-source
                                innovation but crave a space that feels human, creative, and accessible.
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-4 mt-4">
                            <button className="flex items-center justify-center rounded-xl h-12 px-6 bg-primary text-white text-base font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all cursor-pointer">
                                Our Manifesto
                            </button>
                            <button className="flex items-center justify-center rounded-xl h-12 px-6 border border-primary/30 text-primary text-base font-bold hover:bg-primary/5 transition-all cursor-pointer">
                                Meet the Team
                            </button>
                        </div>
                    </div>
                </div>
            </section>

            {/* Mission Values Grid */}
            <section className="max-w-7xl mx-auto px-4 md:px-8 py-16 border-t border-neutral-200">
                <div className="flex flex-col gap-4 text-center items-center mb-10">
                    <h2 className="text-3xl md:text-4xl font-bold leading-tight text-neutral-900 max-w-[720px]">
                        Built on Shared Values
                    </h2>
                    <p className="text-neutral-500 text-base font-normal leading-normal max-w-[600px]">
                        We believe in the power of shared knowledge. Every feature we build is guided by these core principles.
                    </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {/* Card 1 */}
                    <div className="group flex flex-1 gap-5 rounded-xl border border-neutral-200 bg-white p-8 flex-col hover:shadow-xl hover:shadow-primary/5 hover:border-primary/40 transition-all duration-300">
                        <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                            <span className="material-symbols-outlined text-[28px]">code</span>
                        </div>
                        <div className="flex flex-col gap-2">
                            <h3 className="text-xl font-bold leading-tight text-neutral-900">Open Source Heart</h3>
                            <p className="text-neutral-500 text-sm font-normal leading-relaxed">
                                Empowering engineers to build better together through accessible code, shared blueprints, and transparent tools.
                            </p>
                        </div>
                    </div>

                    {/* Card 2 */}
                    <div className="group flex flex-1 gap-5 rounded-xl border border-neutral-200 bg-white p-8 flex-col hover:shadow-xl hover:shadow-primary/5 hover:border-primary/40 transition-all duration-300">
                        <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                            <span className="material-symbols-outlined text-[28px]">diversity_3</span>
                        </div>
                        <div className="flex flex-col gap-2">
                            <h3 className="text-xl font-bold leading-tight text-neutral-900">Community First</h3>
                            <p className="text-neutral-500 text-sm font-normal leading-relaxed">
                                Fostering a supportive environment where every contribution is valued, from the first PR to major architectural shifts.
                            </p>
                        </div>
                    </div>

                    {/* Card 3 */}
                    <div className="group flex flex-1 gap-5 rounded-xl border border-neutral-200 bg-white p-8 flex-col hover:shadow-xl hover:shadow-primary/5 hover:border-primary/40 transition-all duration-300">
                        <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                            <span className="material-symbols-outlined text-[28px]">brush</span>
                        </div>
                        <div className="flex flex-col gap-2">
                            <h3 className="text-xl font-bold leading-tight text-neutral-900">Artistic Vision</h3>
                            <p className="text-neutral-500 text-sm font-normal leading-relaxed">
                                Blending technical precision with an aesthetic approach. We believe beautiful tools inspire beautiful work.
                            </p>
                        </div>
                    </div>
                </div>
            </section>

            {/* Feature Highlight / Narrative Section */}
            <section className="max-w-7xl mx-auto px-4 md:px-8 py-10">
                <div className="flex flex-col-reverse md:flex-row items-center justify-between gap-12 rounded-2xl bg-neutral-100 p-8 md:p-12">
                    <div className="flex flex-1 flex-col gap-6">
                        <div className="flex flex-col gap-3">
                            <h3 className="text-2xl md:text-3xl font-bold leading-tight text-neutral-900">
                                The Open Source Narrative
                            </h3>
                            <p className="text-neutral-500 text-base font-normal leading-relaxed">
                                From humble beginnings as a small repository to a thriving ecosystem, UniCare has always been about more
                                than just code. It&apos;s about the people behind the screen. We are crafting a narrative where technology
                                serves humanity with grace and style.
                            </p>
                        </div>
                        <button className="flex items-center justify-center rounded-xl h-10 px-6 bg-neutral-900 text-white text-sm font-medium w-fit hover:opacity-90 transition-opacity cursor-pointer">
                            Read Full Narrative
                            <span className="material-symbols-outlined text-sm ml-2">arrow_forward</span>
                        </button>
                    </div>
                    <div className="relative w-full md:w-1/2 aspect-video rounded-xl overflow-hidden shadow-lg">
                        <Image
                            src="/about-narrative.png"
                            alt="Group of diverse engineers collaborating around a table"
                            fill
                            className="object-cover"
                        />
                    </div>
                </div>
            </section>

            {/* Stats Section */}
            {/* <section className="max-w-7xl mx-auto px-4 md:px-8 py-10">
                <div className="flex flex-wrap gap-4">
                    {[
                        { icon: "engineering", label: "Active Engineers", value: "10k+" },
                        { icon: "folder_open", label: "Resources Shared", value: "50k+" },
                        { icon: "event", label: "Community Events", value: "120" },
                        { icon: "public", label: "Global Reach", value: "30+ Countries" },
                    ].map((stat) => (
                        <div
                            key={stat.label}
                            className="flex min-w-[158px] flex-1 flex-col gap-2 rounded-xl p-6 border border-neutral-200 bg-white"
                        >
                            <div className="flex items-center gap-2 text-primary mb-2">
                                <span className="material-symbols-outlined">{stat.icon}</span>
                            </div>
                            <p className="text-neutral-500 text-sm font-medium leading-normal uppercase tracking-wider">
                                {stat.label}
                            </p>
                            <p className="text-neutral-900 text-3xl font-bold leading-tight">{stat.value}</p>
                        </div>
                    ))}
                </div>
            </section> */}
        </div>
    );
}
